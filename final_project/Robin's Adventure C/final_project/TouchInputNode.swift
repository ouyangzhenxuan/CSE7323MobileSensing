...
class TouchInputNode : SKSpriteNode {
    ...
    var itemName:[String] = ["no","no","no","no","no","no"]
    var selected:Int = -1
    ...
    func setupControls(size : CGSize) {
        ...
        addButton(button: buttonPause,
                  position: CGPoint(x: -(size.width / 3 ) - 50, y: size.height / 3-10),
                  name: "pause",
                  scale: 0.8)
        ...
        addButton(button: buttonDirRight,
                  position: CGPoint(x: -(size.width / 3 ) + 25, y: -size.height / 3+10),
                  name: "right",
                  scale: 0.8)
    ...
    func setinventory(tex:SKTexture,category:UInt32,name:String) -> Bool{
        var full = false
...
//                    item.physicsBody?.contactTestBitMask = 1
                    item.name=self.itemName[self.selected]
            ...
            for button in [ buttonDirLeft, buttonDirRight, buttonA,inventory1_item,inventory2_item,inventory3_item,inventory4_item,inventory5_item,inventory6_item,motionbutton,motionbutton2,buttonB,buttonPause,
                ...
                if pressedButtons.index(of: button) == nil {
                    if(!self.inventory.contains(button)){
                            if(button.name == "T" || button.name == "M"){
                                button.alpha = 0
                            }
                            else{
                                button.alpha = alphaUnpressed}}
