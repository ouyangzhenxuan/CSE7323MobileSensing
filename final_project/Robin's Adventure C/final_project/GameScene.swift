...
class GameScene: SKScene,ControlInputDelegate,SKPhysicsContactDelegate{
    ...
    var priest: SKSpriteNode?
    var doctor: SKSpriteNode?
    var doctor_2:SKSpriteNode?
    ...
    func eatItem(){
        if(self.touchControlNode?.checkItemCategory()==2){
            if let item = self.touchControlNode?.useItem(){
                ...
                let duration_action = SKAction.wait(forDuration: 10)
...
//        self.ninjaEatFood()
    ...
    func priest_quest(){
        ...
        self.priest = (childNode(withName: "priest") as? SKSpriteNode)!
    ...
    func mission_doctor(){
        ...
        doctor = childNode(withName: "doctor") as? SKSpriteNode
    ...
    func mission_brother(){
        if(meet_doctor){
           ...
           doctor_2 = childNode(withName: "doctor_2") as? SKSpriteNode
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if(self.startedLao){
            if(Int((self.player?.position.x)!) > -4571){
        ...
        if(mission_priest){
            if(Int((self.player?.position.x)!) > 565){
        ...
        if(meet_doctor){
            if(Int((self.player?.position.x)!) > 2544){
                
                doctor?.texture=SKTexture(imageNamed: "doctor_right")
            }else{
                doctor?.texture=SKTexture(imageNamed: "doctor")
            }
        }
        if(meet_brother){
            if(Int((self.player?.position.x)!) > 3910){
                
                doctor_2?.texture=SKTexture(imageNamed: "doctor_right")
            }else{
                doctor_2?.texture=SKTexture(imageNamed: "doctor")
            }
        }
        if(startedSun){
            if(Int((self.player?.position.x)!) > 3316){
                
                sun?.texture=SKTexture(imageNamed: "knight_right")
            }else{
                sun?.texture=SKTexture(imageNamed: "knight_left")
            }
        }
        if(self.left){
...
//                player.position.x = (self.player?.position.x)!+2
    ...
    func didBegin(_ contact: SKPhysicsContact){
        ...
        if contact.bodyA.categoryBitMask == 1 && contact.bodyB.node?.name=="船上甲板" {
            let speak = SKAction.run{
                self.callText(text: "Robin: Oh, I finnaly get the destination, let's back to menu")
            }
            let end = SKAction.run {
                self.backToMenu()
            }
            let wait = SKAction.wait(forDuration: 2)
            self.run(SKAction.sequence([speak,wait,end]))
        }
    
        if contact.bodyB.categoryBitMask == 1 && contact.bodyA.node?.name=="船上甲板" {
            let speak = SKAction.run{
                self.callText(text: "Robin: Oh, I finnaly get the destination, let's back to menu")
            }
            let end = SKAction.run {
        ...
        if contact.bodyA.node?.name == "Ninja" && contact.bodyB.categoryBitMask == 22 {
            ...
            let nj:SKSpriteNode = contact.bodyA.node as! SKSpriteNode
            nj.texture = SKTexture.init(imageNamed: "ninja_right")
        ...
        if contact.bodyA.categoryBitMask == 22 && contact.bodyB.node?.name == "Ninja" {
            ...
            let nj:SKSpriteNode = contact.bodyB.node as! SKSpriteNode
            nj.texture = SKTexture.init(imageNamed: "ninja_right")
