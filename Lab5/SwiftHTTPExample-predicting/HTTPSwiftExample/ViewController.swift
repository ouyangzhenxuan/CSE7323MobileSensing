...
class ViewController: UIViewController, URLSessionDelegate, UIPickerViewDelegate,
    ...
    @objc func buttonDown(_ sender: UIButton) {
        ...
        startMotionUpdates()
        timer = Timer.scheduledTimer(timeInterval: 0.3, target: self, selector: #selector(fire), userInfo: nil, repeats: true)
    }
    ...
    func handleMotion(_ motionData:CMDeviceMotion?, error:Error?){
        if let accel = motionData?.userAcceleration {
            ...
            if (self.handleMotionCount >= 100){
                ...
                // stop motion updates (predict only once when each time hold-down the button)
                stopMotionUpdates()
            }
        }
    }
